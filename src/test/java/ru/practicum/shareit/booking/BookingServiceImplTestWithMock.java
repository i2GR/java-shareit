...
class BookingServiceImplTestWithMock {
    ...
    private BookingDto booking1Dto, booking2Dto;
    private BookingResponseDto response1Dto, response2Dto;
    ...
    void approve_whenBookingStatusAlreadyApproved_thenBadRequestException() {
        ...
        //then
        assertEquals(format("bad status of booking %s", BookingStatus.APPROVED.toString()), bre.getMessage());
        Mockito.verify(bookingStorage, only()).findById(anyLong());
    }
    ...
    private void setupUsersAndItems() {
        user1 = User.builder().id(1L).name("user1").email("user1@host.dom").build();
    ...
    private void setupEntityDtos(LocalDateTime start, LocalDateTime end) {
        ...
        booking2Dto = BookingDto.builder()
                ...
                .itemId(2L)
                .build();
        response2Dto = BookingResponseDto.builder()
                .id(2L)
                .start(start)
                .end(end)
                .status(BookingStatus.WAITING)
                .item(BookingResponseDto.ItemDto.builder().id(2L).name("name2").build())
